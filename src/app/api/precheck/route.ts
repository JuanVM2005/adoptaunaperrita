...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!hoursAlone || !biteHandling || !routine) {
      return NextResponse.json(
        {
          pass: false,
          score: 0,
          summary: "Completa las 3 preguntas para poder evaluar.",
    ...
    const answers: PrecheckAnswers = { hoursAlone, biteHandling, routine };
    ...
    const res = await client.responses.create({
      ...
      input: [
        ...
        {
          role: "user",
          content: [{ type: "input_text", text: buildPrecheckUserPrompt(answers) }],
        },
      ],
      text: {
        format: {
